...
#     about_window.title("Thông tin sản phẩm và người làm ra sản phẩm")
#
...
def show_about_info():
    ...
    creator_label = tk.Label(info_frame, text="Người làm ra sản phẩm:", font=("Arial", 12, "bold"))
...
def play_audio():
    ...
    if audio_file:
        ...
        if os.path.exists(audio_file):
            try:
                # Sử dụng thư viện pygame để phát âm thanh từ tệp âm thanh cơ sở dữ liệu
                pygame.mixer.music.load(audio_file)
                pygame.mixer.music.play()
            except Exception as e:
                messagebox.showerror("Lỗi", f"Lỗi khi phát âm thanh từ cơ sở dữ liệu: {str(e)}")
        else:
            messagebox.showinfo("Thông báo", "Tệp âm thanh từ cơ sở dữ liệu không tồn tại.")
    else:
        # Nếu tên tệp âm thanh không tồn tại trong cơ sở dữ liệu, thử phát âm thanh từ URL gốc
        selected_word = vocabulary_tree.item(selected_item, "values")[0]  # Lấy từ vựng
        if selected_word:
            url = f"https://translate.google.com.vn/translate_tts?ie=UTF-8&q={selected_word}&tl=en&client=tw-ob"
            try:
                # Sử dụng thư viện playsound để phát âm thanh từ URL gốc
                playsound.playsound(url)
            except Exception as e:
                messagebox.showerror("Lỗi", f"Lỗi khi phát âm thanh từ URL gốc: {str(e)}")
        else:
            messagebox.showinfo("Thông báo", "Từ vựng này không có tên tệp âm thanh trong cơ sở dữ liệu và không thể phát từ URL gốc.")
...
def display_word_details(word):
    ...
    if word_details:
        ...
        label_word = ttk.Label(details_frame, text="Từ vựng:", font=("Arial", 12, "bold"), foreground="blue")
        label_word.grid(row=0, column=0, sticky="w")
        label_word_text = ttk.Label(details_frame, text=word_text, font=("Arial", 12, "bold"), foreground="blue")
        label_word_text.grid(row=0, column=1, sticky="w")

        label_meaning = ttk.Label(details_frame, text="Nghĩa:", font=("Arial", 12, "bold"), foreground="green")
        label_meaning.grid(row=1, column=0, sticky="w")
        label_meaning_text = ttk.Label(details_frame, text=meaning, font=("Arial", 10), foreground="green")
        label_meaning_text.grid(row=1, column=1, sticky="w")

        label_word_type = ttk.Label(details_frame, text="Loại từ vựng:", font=("Arial", 12, "bold"), foreground="purple")
        label_word_type.grid(row=2, column=0, sticky="w")
        label_word_type_text = ttk.Label(details_frame, text=word_type, font=("Arial", 10), foreground="purple")
        label_word_type_text.grid(row=2, column=1, sticky="w")

        label_completion = ttk.Label(details_frame, text="Trạng thái hoàn thành:", font=("Arial", 12, "bold"), foreground="red")
        label_completion.grid(row=3, column=0, sticky="w")
        label_completion_text = ttk.Label(details_frame, text='Hoàn thành' if completed else 'Chưa hoàn thành', font=("Arial", 10), foreground="red")
        label_completion_text.grid(row=3, column=1, sticky="w")

        if audio_file:
            # Nếu có tệp âm thanh, hiển thị nút để phát âm thanh
            play_audio_button = ttk.Button(details_frame, text="Nghe Âm Thanh", command=play_audio)
            play_audio_button.grid(row=4, column=0, pady=10, columnspan=2, sticky="w")
        else:
            label_audio_status = ttk.Label(details_frame, text="Từ vựng này chưa có tệp âm thanh.", font=("Arial", 10), foreground="orange")
            label_audio_status.grid(row=4, column=0, pady=10, columnspan=2, sticky="w")
...
play_audio_button = ttk.Button(button_frame, text="Nghe Âm Thanh", command=play_audio)
...
menu_bar.add_cascade(label="Giới thiệu sản phẩm", menu=vocabulary_menu2)
vocabulary_menu2.add_command(label="Thông tin sản phẩm", command=show_about_info)
